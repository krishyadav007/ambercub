...
const AttributeManager = () => {
  ...
  const handleAddAttribute = async (event) => {
    ...
    if (attribute.trim() !== "") {
      try {
        const response = await fetch("/api/attribute/announcement/create", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const handleSaveEdit = async (index) => {
    try {
      const response = await fetch("/api/attribute/announcement/update", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleDelete = async (index) => {
    try {
      const response = await fetch("/api/attribute/announcement/delete", {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <>
          ...
          <form onSubmit={handleAddAttribute} className="mb-4">
            ...
            <button
              type="submit"
              className="bg-[#EAA444] w-full mt-4 text-xl h-12 text-white"
            >
              Add Attribute
            </button>
          </form>
              ...
              <table className="min-w-full bg-white">
                <thead>
                  <tr>
                    ...
                    <th className="py-2 px-4 border-b border-gray-300 w-1/4">
                      Actions
                    </th>
                  </tr>
                </thead>
